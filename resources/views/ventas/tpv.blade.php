    ...
    @if ($cajaAbierta) 
        <div class="row h-100">
            ...
            <div class="col-lg-8 d-flex flex-column h-100">
                <h4 class="mb-3 text-primary"><i class="fas fa-map-marked-alt me-2"></i> Ventas de Terrenos</h4>
                <h4 class="mb-3 text-primary"><i class="fas fa-map-marked-alt me-2"></i> Ventas de Terrenos</h4>
                ...
                <div class="alert alert-info py-2 mb-3">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="row g-2 mt-2">
                         <div class="col-6">
                        <label class="form-label mb-1"><small>Teléfono</small></label>
        <input type="text" id="cliente-telefono" class="form-control form-control-sm" placeholder="Teléfono">
    </div>
    <div class="col-6">
        <label class="form-label mb-1"><small>Dirección</small></label>
        <input type="text" id="cliente-direccion" class="form-control form-control-sm" placeholder="Dirección">
    </div>
</div>
               ...
               {{-- ================== FIN TERRENO ================== --}}
                ...
                <div id="cart" class="flex-grow-1 overflow-auto border rounded p-3 mb-3 bg-light shadow-sm">
                    <p class="text-center text-muted empty-cart-message">Seleccione un terreno y configure las mensualidades para comenzar</p>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const emptyCartMessageHTML = '<p class="text-center text-muted empty-cart-message">Seleccione un terreno y configure las mensualidades para comenzar.</p>';
    ...
    // ✅ PERMITIR VENTA SI HAY TERRENO
    if (precioTerreno > 0 && mensualidades > 0) {
    ...
    // ==========================================================
    // ==========================================================
// CLIENTES (id / cliente + telefono + direccion) ✅ CORREGIDO
// ==========================================================
...
function populateClientDropdown() {
    ...
    if (clients && clients.length > 0) {
        clients.forEach(client => {
            ...
            a.dataset.clientName = client.cliente;              // ✅
            a.dataset.clientTelefono = client.telefono || '';   // ✅
            a.dataset.clientDireccion = client.direccion || ''; // ✅

            a.textContent = client.cliente; // ✅ lo que se muestra
            li.appendChild(a);
...
function updateSelectedClient(id, name, tel = '', dir = '') {
    ...
    // ✅ llenar telefono/direccion
    if (inputTelefono) inputTelefono.value = tel || '';
    if (inputDireccion) inputDireccion.value = dir || '';
}
    ...
    clientDropdownMenu.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('select-client')) {
            updateSelectedClient(
                ...
                e.target.dataset.clientTelefono,
                e.target.dataset.clientDireccion
            );
        }
    });
}
...
if (temporalClientInput) {
    temporalClientInput.addEventListener('input', function() {
        ...
        const match = (clients || []).find(c =>
            ((c.cliente || '').toLowerCase() === typed)
        );

        if (match) {
            selectedClientId = match.id;
            if (inputTelefono) inputTelefono.value = match.telefono || '';
            if (inputDireccion) inputDireccion.value = match.direccion || '';
        } else {
            selectedClientId = null; // si no coincide con cliente existente
            // no borramos teléfono/dirección para permitir capturar manual
        }
    });
}
